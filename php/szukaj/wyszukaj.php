...
if(isset($_POST['term']))
{
    if($_POST['wartosc'] == 1)
    {
        ...
        $operacja = $polaczenie->prepare("SELECT `$jezykID`,idek
                                      FROM tlumaczenia                                  
                                      WHERE `$jezykID` LIKE ? and rodzaj=2
                                      LIMIT 3 ");
        ...
        if(count($a_json) < 3)
        {
            if(count($ideki) > 0)
            {
                ...
                $operacja = $polaczenie->prepare("SELECT `1`,idek
                                      FROM tlumaczenia
                                      WHERE rodzaj=2 and idek NOT IN ($placeHolders) AND `1` LIKE ?
                                      LIMIT 3 ");
                $indexx = 1;
    ...
    else if($_POST['wartosc'] == 2)
    {
        $term = trim(strip_tags($_POST['term']));
        $a_json = array();
        $a_json_row = array();

        $operacja = $polaczenie->prepare("SELECT id_uzytkownika,login
        ...
        foreach ($wyniki as $wynik) {
            $a_json_row['nazwa'] = $wynik['login'];
            $a_json_row['idek'] = $wynik['id_uzytkownika'];
            $a_json_row['kategoria'] = 'profil';
            $a_json_row['kat'] = 1;
            $a_json_row['img'] = 'FILES/profile/'.$wynik['id_uzytkownika'].'.jpg';
            array_push($a_json, $a_json_row);
